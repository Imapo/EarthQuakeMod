...
namespace RealisticEarthquake.Common.Utils
{
    ...
    public static class CeilingScanner
    {
        ...
        /// <summary>
        /// Землетрясение по ТЗ должно ощущаться только в подземелье и ниже (пещеры, подземелье, пещерный слой,
        /// преисподняя). Main.worldSurface - это глобальная Y-координата тайла, где начинается подземный слой;
        /// всё, что ниже неё - уже не "поверхность".
        /// </summary>
        public static bool IsUndergroundOrBelow(Player player)
        {
            return player.Center.Y / 16f > Main.worldSurface;
        }
    }
}
